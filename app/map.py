...
def find_shortest_path_dij(map, start, end):
    ...
    """ Set startNode to 0, all other nodes to infinity """
    for city in map.keys():
        dist_to_city_node[city] = ((start, 0) if city == start else (start, float('inf')))


    """ Add the startNode to un_settled_cities """
    un_settled_cities.append(start)


    """ Find the shortest path (Dijkstra's Algorithm) """
    while len(un_settled_cities):
        un_settled_cities = sorted(un_settled_cities, key = lambda c: dist_to_city_node[c][1])
        city = un_settled_cities.pop(0)
        visited_cities.add(city)
        neighbors = map[city]

        for neighbor in neighbors:
            next_city, dist = neighbor

            new_dist = dist + dist_to_city_node[city][1]
            curr_dist = dist_to_city_node[next_city][1]

            min_dist = min(curr_dist, new_dist)
            min_city = city if min_dist != curr_dist else dist_to_city_node[next_city][0]

            dist_to_city_node[next_city] = (min_city, min_dist)

            if next_city not in visited_cities:
                un_settled_cities.append(next_city)
